defmodule NervesPhotos.ImageLoaderTest do
  ...
  setup do
    {:ok, _pid} =
      start_supervised({ImageLoader, put_fn: fn _key, _bytes -> {:ok, 100, 100} end})

    :ok
  end

  test "load/2 sends {:image_loaded, key, width, height} to caller on success" do
    asset = {GoodSource, "asset-1", %{}, %{date: nil, location: nil}}
    ImageLoader.load(asset, self())
    assert_receive {:image_loaded, "asset-1", 100, 100}, 500
  end

  test "load/2 uses source_id as default stream key" do
    asset = {GoodSource, "asset-1", %{}, %{date: nil, location: nil}}
    ImageLoader.load(asset, self())
    assert_receive {:image_loaded, "asset-1", _, _}, 500
  end
  ...
  test "load/3 accepts a custom stream key" do
    ...
    assert_receive {:image_loaded, "photo:custom", 100, 100}, 500
  end
end
